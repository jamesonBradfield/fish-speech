# Fish Speech — DirectML / Windows fork

This fork of [fishaudio/fish-speech](https://github.com/fishaudio/fish-speech) (v1.5.1 base) adds:

1. **ONNX Runtime + DirectML inference engine** for AMD GPUs on Windows — `tools/onnx_export_full.py` exports the DualAR text2semantic transformer (slow/fast/emb graphs, step-wise KV-cache, fp32) and `tools/onnx_tts.py` runs TTS through the `DmlExecutionProvider` (numpy sampling, torch-CPU batched prefill, DML decoder via the ConvTranspose1d→2D graph rewrite in `tools/onnx_fix_convtranspose.py`). Measured ~24x faster than CPU torch on an RX 7700 XT; logits parity with torch `forward_generate` verified (~2e-5).
   ```bash
   python tools/onnx_export_full.py --checkpoint checkpoints/fish-speech-1.5 --out-dir onnx_artifacts
   python tools/onnx_tts.py --checkpoint checkpoints/fish-speech-1.5 --onnx-dir onnx_artifacts \
     --text "..." --reference_audio ref.wav --reference_text "..." --output out.wav
   ```
   Requires `pip install onnx onnxruntime-directml soundfile onnxscript`. Exported graphs live in `onnx_artifacts/` (git-ignored; regenerated by the export script).

2. **Cheap Windows TTS for lo-fi / retro voices** — `tools/cheap_tts.py` renders text with the built-in Microsoft SAPI voice (Zira, female) and crunches it to a 1999-era game-audio aesthetic (11 kHz, 8-bit, bandpass, dynamics crush) via `tools/lofi_1999.py`. No GPU required.
   ```bash
   python tools/cheap_tts.py --text "I am the master computer." --output out.wav
   ```

3. **Windows audio-loading fix** — `fish_speech/inference_engine/reference_loader.py` now prefers the `soundfile` backend for reference audio, working around torchaudio ≥2.9's hard dependency on TorchCodec (which requires FFmpeg shared DLLs that are typically absent on Windows). Submitted upstream.
